...
class QuoteBox extends React.Component {
  ...
  render() {
    return (
      <div id="quote-box" class="container rounded px-5 py-4 bg-light">
        <p id="text" class="col text-center lead"><strong>{this.state.text}</strong></p>
        <p id="author" class="col text-end lead">{this.state.author}</p>
        <div class="d-flex justify-content-between">
          <a href={this.state.twitterURL} id="tweet-quote"><button type="button" class="text btn btn-primary"><i class="bi bi-twitter"></i></button></a>
          <button type="button" id="new-quote" onClick={this.handleClick} class="text btn btn-primary">New quote</button>
        </div>
      </div>
    )
  };
